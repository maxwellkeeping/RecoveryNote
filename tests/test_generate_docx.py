...
class TestDocGeneration:
    ...
    def test_financial_table_amount(self, generated_doc, sample_data):
        t1 = generated_doc.tables[1]
        assert t1.rows[1].cells[4].text.strip() == "$5000.00"

    def test_header_table_solution_ci_row(self, generated_doc, sample_data):
        t0 = generated_doc.tables[0]
        assert t0.rows[5].cells[0].text.strip() == "Solution CI:"
        assert t0.rows[5].cells[1].text.strip() == sample_data["SOLUTION_CI"]

    def test_header_table_solution_ci_row_present_when_empty(self, sample_data):
        sample_data["SOLUTION_CI"] = ""
        path = generate_docx.generate(sample_data)
        doc = Document(path)
        t0 = doc.tables[0]
        assert t0.rows[5].cells[0].text.strip() == "Solution CI:"
        assert t0.rows[5].cells[1].text.strip() == ""
        os.unlink(path)

    def test_header_table_solution_ci_row_style_matches_above(self, generated_doc):
        t0 = generated_doc.tables[0]
        assert (
            t0.rows[5].cells[0].paragraphs[0].style
            == t0.rows[4].cells[0].paragraphs[0].style
        )
        assert (
            t0.rows[5].cells[1].paragraphs[0].style
            == t0.rows[4].cells[1].paragraphs[0].style
        )
