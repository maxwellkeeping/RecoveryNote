...
sanitize_name = lambda s: re.sub(r"[^0-9A-Za-z]+", '_', s).strip('_')
...
class TestCascadeLogic:
    ...
    def test_cascade_parent_is_dict(self):
        ...
        assert isinstance(parent_lookup, dict), "Parent lookup should be a dict for cascading"
    ...
    def test_cascade_parent_has_expected_keys(self):
        ...
        parent_mapped = mapping.get(parent_label)
        parent_lookup = lookups.get(parent_mapped) if parent_mapped else lookups.get(parent_label)
        expected_services = ['PROCUREMENT-SOFTWARE', 'PROCUREMENT-HARDWARE', 'MAINFRAME']
    ...
    def test_parent_field_renders_as_plain_dropdown(self):
        ...
        _, lookups, cascade_field_map, mapping = load_field_groups_for_test()
        # ITS  SERVICE is the parent — its own lookup resolves to a dict
        parent_mapped = mapping.get('ITS  SERVICE')
        lookup_val = lookups.get(parent_mapped) if parent_mapped else lookups.get('ITS  SERVICE')
