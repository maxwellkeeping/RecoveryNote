...
GROUP_KEYWORDS = {
    ...
    'Agreement': ['agreement', 'agreement no', 'agreement#', 'agreement id', 'contract', 'id'],
...
def main():
    ...
    for r in range(1, 11):
        ...
        if len(non_empty) >= 3 and not any('agreements log' in str(c).lower() for c in non_empty):
            header_row_idx = r
            break
    ...
    sample = {}
    first_data_row = None
    for row in sheet.iter_rows(min_row=header_row_idx+1, max_row=header_row_idx+1, values_only=True):
        first_data_row = row
        break
    if first_data_row:
